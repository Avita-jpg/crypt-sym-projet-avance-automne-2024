...
def Alice(p,g,queueAlice,queueBob,queueCle):
   ...
   print(f"\n[Connu d'Alice, Bob et de potentiels espions(Eve)] --> A = {A}")
   queueAlice.put(A)
...
def Bob(p,g,queueAlice,queueBob):
   ...
   print(f"\n[Connu d'Alice, Bob et de potentiels espions(Eve)] --> B = {B}")
   queueBob.put(B)
...
def main() :
  ...
  parser.add_argument('-i', '--input', type=str, required=True, help='Fichier d\'entrée avec les paramètres p et g.')
  parser.add_argument('-o', '--output', type=str, required=True, help='Fichier de sortie pour la clé générée.')
  ...
  queueAlice = queue.Queue()  # file d'attente pour A
  queueBob = queue.Queue()  # file d'attente pour B
  queueCle = queue.Queue() # file d'attente pour la clé
